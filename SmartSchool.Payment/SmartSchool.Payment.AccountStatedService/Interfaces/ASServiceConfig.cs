...
{
    public class ASServiceConfig
    {
        public ASServiceConfig()
        {
            _base_xml = DSXmlHelper.LoadXml("<ASServiceConfig/>");
        }

        internal ASServiceConfig(XmlElement conf)
        {
            _base_xml = conf;
        }
        ...
        public XmlElement Content
        {
            ...
            set
            {
                if (value.LocalName != "Content")
                    throw new ArgumentException("Xml 根名稱必需是「Content」。");
