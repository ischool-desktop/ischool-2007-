...
namespace SmartSchool.Evaluation.ImportExport
{
    ...
    class ImportSchoolYearEntryScore : ImportProcess
    {
        ...
        public ImportSchoolYearEntryScore()
        {
            ...
            foreach ( string field in new string[] { "學年度", "成績年級", "學業", "體育", "國防通識", "健康與護理", "實習科目", "德行" } )
            {
        ...
        private void ImportSchoolYearEntryScore_DataImport(object sender, DataImportEventArgs e)
        {
            ...
            foreach ( string id in id_Rows.Keys )
            {
                ...
                foreach ( int sy in schoolYearImportScore.Keys )
                {
                    ...
                    //匯入學習類成績
                    #region 匯入學習類成績
                    if ( schoolYearImportScore[sy].ContainsKey("學業") || schoolYearImportScore[sy].ContainsKey("體育") || schoolYearImportScore[sy].ContainsKey("國防通識") || schoolYearImportScore[sy].ContainsKey("健康與護理") || schoolYearImportScore[sy].ContainsKey("實習科目") )
                    {
                        #region 匯入分項成績
                        string scoreId = "";
                        bool hasEntry = false;
