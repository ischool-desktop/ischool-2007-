...
namespace SmartSchool.Evaluation.ImportExport
{
    ...
    class ImportSemesterSubjectScore :SmartSchool.API.PlugIn.Import.Importer
    {
        ...
        public override void InitializeImport(SmartSchool.API.PlugIn.Import.ImportWizard wizard)
        {
            ...
            wizard.ImportableFields.AddRange("科目", "科目級別", "學年度", "學期", "學分數", "分項類別", "成績年級", "必選修", "校部訂", "原始成績", "補考成績", "重修成績", "手動調整成績", "學年調整成績", "取得學分", "不計學分", "不需評分", "註記");
            wizard.RequiredFields.AddRange("科目", "科目級別", "學年度", "學期", "成績年級");
            ...
            wizard.ValidateStart += delegate(object sender, SmartSchool.API.PlugIn.Import.ValidateStartEventArgs e)
            {
                ...
                Common.MultiThreadWorker<StudentRecord> loader = new SmartSchool.Common.MultiThreadWorker<StudentRecord>();
                loader.MaxThreads = 3;
                loader.PackageSize = 250;
                loader.PackageWorker += delegate(object sender1, SmartSchool.Common.PackageWorkEventArgs<StudentRecord> e1)
                {
                ...
                foreach ( string field in e.SelectFields )
                {
                    ...
                    switch ( field )
                    {
                        ...
                        case "校部訂":
                            ...
                            break;
                        case "分項類別":
                            if ( value != "學業" && value != "體育" && value != "國防通識" && value != "健康與護理" && value != "實習科目" )
                            {
                                inputFormatPass &= false;
                                e.ErrorFields.Add(field, "必須填入 學業、體育、國防通識、健康與護理或實習科目");
                            }
                            break;
                    }
                }
