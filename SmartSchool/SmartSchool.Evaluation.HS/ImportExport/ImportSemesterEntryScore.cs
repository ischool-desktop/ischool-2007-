...
namespace SmartSchool.Evaluation.ImportExport
{
    ...
    class ImportSemesterEntryScore : ImportProcess
    {
        ...
        public ImportSemesterEntryScore()
        {
            ...
            foreach ( string field in new string[] { "學年度", "學期", "成績年級", "學業", "體育", "國防通識", "健康與護理", "實習科目", "德行" } )
            {
            ...
            foreach ( string id in id_Rows.Keys )
            {
                ...
                foreach ( RowData row in id_Rows[id] )
                {
                    ...
                    foreach ( string key in row.Keys )
                    {
                        ...
                        if ( e.ImportFields.Contains(key) && key == "學業" || key == "體育" || key == "國防通識" || key == "健康與護理" || key == "實習科目" || key == "德行" )
                        {
                            bool import = true;
                            decimal? score=null  ;
                            decimal d;
                ...
                foreach ( int sy in semesterImportScore.Keys )
                {
                    foreach ( int se in semesterImportScore[sy].Keys )
                    {
                        ...
                        //匯入學習類成績
                        #region 匯入學習類成績
                        if ( semesterImportScore[sy][se].ContainsKey("學業") || semesterImportScore[sy][se].ContainsKey("體育") || semesterImportScore[sy][se].ContainsKey("國防通識") || semesterImportScore[sy][se].ContainsKey("健康與護理") || semesterImportScore[sy][se].ContainsKey("實習科目") )
                        {
                            #region 匯入分項成績
                            string scoreId = "";
                            bool hasEntry = false;
                            XmlElement entryScore = doc.CreateElement("SemesterEntryScore");
                            foreach ( string key in new string[] { "學業", "體育", "國防通識", "健康與護理", "實習科目" } )
                            {
