    ...
    public partial class SchoolYearScorePalmerworm : UserControl, SmartSchool.Customization.PlugIn.ExtendedContent.IContentItem, IPreference
    {
        ...
        private void FillScore()
        {
            ...
            foreach (string var in Credit.Keys)
            {
                ...
                subItems[8] = Credit[var].ToString();
            }
            #endregion
        ...
        private void LoadPreference()
        {
            ...
            {
                string[] attributeList = new string[] { "col健康與護理Visible", "col國防通識Visible", "col實得學分Visible", "col實習科目Visible", "col德行成績Visible", "col學業成績Visible", "col體育成績Visible" };
                ColumnHeader[] columnList = new ColumnHeader[] { col健康與護理, col國防通識, col實得學分, col實習科目, col德行成績, col學業成績, col體育成績 };
                ...
                //預設顯示欄位
                lvColumnManager1.HideColumn(new ColumnHeader[] { col體育成績, col國防通識, col健康與護理, col實習科目 });
        }
        ...
        private void listView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                SmartSchool.MotherForm.Instance.SetWaitCursor();
                ScoreEditor.SchoolYearScoreEditor editor = new ScoreEditor.SchoolYearScoreEditor(listView1.SelectedItems[0].SubItems[0].Text, listView1.SelectedItems[0].SubItems[1].Text, _CurrentID);
                editor.FormClosed += new FormClosedEventHandler(editor_FormClosed);
                SmartSchool.MotherForm.Instance.ResetWaitCursor();
                editor.ShowDialog();
            }
        }
