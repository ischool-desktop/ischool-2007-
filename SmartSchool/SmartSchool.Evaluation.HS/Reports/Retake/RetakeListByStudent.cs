...
namespace SmartSchool.Evaluation.Reports
{
    class RetakeListByStudent
    {
        ...
        public RetakeListByStudent()
        {
            ...
            bkwNotPassComputer.DoWork += new DoWorkEventHandler(bkwNotPassComputerByStudent_DoWork);
        ...
        private void bkwNotPassComputer_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (e.Error == null)
            {
                ...
                MotherForm.Instance.SetBarMessage(reportName + "產生發生未預期錯誤。");
        }
        ...
        private void bkwNotPassComputerByStudent_DoWork(object sender, DoWorkEventArgs e)
        {
                    ...
                    foreach (XmlElement scoreElement in resp.GetContent().GetElements("SemesterSubjectScore[RefStudentId='" + student.ID + "']"))
                    {
                        ...
                        foreach (XmlElement subjectScoreElement in helper.GetElements("ScoreInfo/SemesterSubjectScoreInfo/Subject"))
                        {
                            ...
                            if (subjectScoreElement.GetAttribute("是否取得學分") == "是" || studentPassedList.Contains(subject))//如果該科目有取得學分獲該科目在其他學期已取得學分
                            {
                                ...
                                //從未取得學分科目清單中移除
                                if (notPassList.ContainsKey(student) && notPassList[student].ContainsKey(subject))
                                {
                                    notPassList[student].Remove(subject);
                                }
                            }
                            else
                            {
                                ...
                                //加入至未取得學分科目清單
                                if (!notPassList.ContainsKey(student))
                                    notPassList.Add(student, new Dictionary<string, XmlElement>());
                                if (!notPassList[student].ContainsKey(subject))
                                    notPassList[student].Add(subject, subjectScoreElement);
                            }
                        }
                    }
            ...
            report.Open(new MemoryStream(Properties.Resources.學生重修科目清單), FileFormatType.Excel2003);
            ...
            foreach (BriefStudentData studentKey in notPassList.Keys)
            {
                if (notPassList[studentKey].Count > 0)
                {
                    foreach (string subject in notPassList[studentKey].Keys)
                    {
                        ...
                        report.Worksheets[0].Cells[index, 4].PutValue(subjectElement.GetAttribute("學年度"));//學年度
                        report.Worksheets[0].Cells[index, 5].PutValue(subjectElement.GetAttribute("學期"));//學期
                        report.Worksheets[0].Cells[index, 6].PutValue(doc.DocumentElement.GetAttribute("科目") + (level == 0 ? "" : " " + GetNumber(level)));//科目
                        report.Worksheets[0].Cells[index, 7].PutValue(doc.DocumentElement.GetAttribute("學分數"));//學分數
                        ...
                        report.Worksheets[0].Cells[index, 8].PutValue("" + maxScore);//學期成績
                        ...
                        if (ScoreCalcRule.ScoreCalcRule.Instance.GetStudentScoreCalcRuleInfo(studentKey.ID) != null && int.TryParse(subjectElement.GetAttribute("年級"), out gradeyear))
                            report.Worksheets[0].Cells[index, 9].PutValue(ScoreCalcRule.ScoreCalcRule.Instance.GetStudentScoreCalcRuleInfo(studentKey.ID).GetStudentPassScore(studentKey, gradeyear));//及格基分
                        else
