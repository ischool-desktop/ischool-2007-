    ...
    abstract class AbstractExportUpdateRecord : ExportProcess
    {
        ...
        private void ExportUpdateRecord_ExportPackage(object sender, ExportPackageEventArgs e)
        {
            ...
            foreach ( SmartSchool.Customization.Data.StudentRecord stu in students )
            {
                ...
                {
                    if ( var.UpdateRecordType != Type ) continue;
                    ...
                    foreach ( string field in e.ExportFields )
                    {
                        if ( ExportableFields.Contains(field) )
                        {
                            switch ( field )
                            {
                                ...
                                case "更正後資料":
                                    {
                                        if (var.Detail.SelectSingleNode("ContextInfo/ContextInfo/NewData") != null)
                                            row.Add(field, var.Detail.SelectSingleNode("ContextInfo/ContextInfo/NewData").InnerText);
                                        else
                                            row.Add(field, "");
                                        break;
                                    }
