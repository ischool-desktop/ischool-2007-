    ...
    abstract class AbstractImportUpdateRecord : ImportProcess
    {
        ...
        void ImportNewStudentsUpdateRecord_BeginValidate(object sender, BeginValidateEventArgs e)
        {
            ...
            foreach ( StudentRecord stu in students )
            {
                ...
                {
                    if ( uinfo.UpdateRecordType == Type )
                    {
                ...
                if ( _NewStudentUpdateRecordInfoList.ContainsKey(row.ID) )
                {
                    ...
                    else
                    {
                        ...
                        foreach ( string field in fieldNameMapping.Keys )
                        {
                            ...
                            {
                                XmlNode importNode = contextInfo.CreateElement(field);
                                importNode.InnerText = row.ContainsKey(fieldname) ? row[fieldname] : ( oldElement.SelectSingleNode("ContextInfo/ContextInfo/" + field) == null ? "" : oldElement.SelectSingleNode("ContextInfo/ContextInfo/" + field).InnerText );
                                root.AppendChild(importNode);
                            }
                            else
                            {
        ...
        {
            SmartSchool.Customization.PlugIn.Global.SetStatusBarMessage(Type+"匯入完成。");
        }
